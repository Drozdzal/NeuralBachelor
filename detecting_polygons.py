...
def find_chessboard_corners(image: np.ndarray,
    ...
    if visualize: utilitus.visualize_keypoints(image, corners)
    ...
    if visualize: utilitus.visualize_hull(image, quad, corners)
    ...
    if visualize: utilitus.visualize_chessboard(image, pattern_size, corners)

    return True, corners

image=cv2.imread('testo.jpg')
find_chessboard_corners(image,(8,8),visualize=True)